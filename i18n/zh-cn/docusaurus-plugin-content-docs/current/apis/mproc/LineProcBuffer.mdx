...
### <IconObjType type="method"/> `force_stop` {#-force_stop}
...
调用该方法，将会导致`receive()`强迫返回`False`。此后，所有向该缓存发送信息的镜像都会被分离（detached）。

需要注意的是，该方法并不安全，而且有可能造成带有镜像的子进程阻塞。如无必要，更推荐使用`stop_all_mirrors()`、而非此方法。

调用该方法就意味着、该缓存的多进程功能被释放，除非再调用`self.reset_states()`。

如果用户打算将所有的子进程透过`Process.terminate()`或`Process.kill()`中断，则建议使用该方法停止主缓存。
