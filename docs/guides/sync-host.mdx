...
In some cases, we need to synchornize the messages among differrent devices. Each device could not get access to the file system of the other devices. The synchronization needs to be implemented by the web service. We design the [`LineHostBuffer`](../apis/host/LineHostBuffer.mdx) based on [`flask`<IconExternalLink/>][doc-flask] and [`flask-restful`<IconExternalLink/>][doc-flask-restful], and the [`LineHostMirror`](../apis/host/LineHostMirror.mdx) based on [`urllib3`<IconExternalLink/>][doc-urllib3].
