...
<p>
  ...
  <Splitter />
  <SourceURL url="file.py#L32">
    <InlineIcon icon={octFileCode16} width='1.35rem' style={{"verticalAlign": "-0.3rem"}} /> Source
  </SourceURL>
</p>

```python
fbuf = syncstream.file.LineFileBuffer(file_path, maxlen=20, tmp_id='tmp')
fbuf = syncstream.LineFileBuffer(...)  # Alias
```
...
|  Argument  |  Type  |  <center>Description</center>  |
| :--------: | :----: | :----------------------------- |
| `lines`    | `[str]`  | A sequence of strs to be written in the log files. Users could catch any line item, and extract any specific information from it. |

### `read` {#read}

```python
lines = fbuf.read(size=None)
```
