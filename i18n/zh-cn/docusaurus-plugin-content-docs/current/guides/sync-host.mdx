...
某些情形下，需要在不同的设备之间同步信息。设备之间互相无法访问对方的文件系统，这使得需要使用网络服务来实现同步。基于[`flask`<IconExternalLink/>][doc-flask]，本项目实现了[`LineHostBuffer`](../apis/host/LineHostBuffer.mdx)；基于[`urllib3`<IconExternalLink/>][doc-urllib3]，本项目实现了[`LineHostMirror`](../apis/host/LineHostMirror.mdx)。
